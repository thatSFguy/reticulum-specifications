...
It happened again on 2026-08-31. §4.6 documented an `rrc.hub` announce `app_data` shape as a **MUST**, sourced from two hub implementations and a census of announcing destinations on a live mesh — no admissible source, no verifier, and directly contradicting §17.5, which fences RRC's own wire format out of this document. The measurement was real; it just measured apps. And the section's load-bearing gotcha turned out to be **wrong**: it claimed a blind msgpack decode of a CBOR map returns the string `"epr"`, when upstream's `display_name_from_app_data` routes that first byte to its raw-UTF-8 branch and raises. Nobody caught it, because a claim about apps has nothing to run against. What replaced it — §4.6 as it now stands, plus `tools/verify_app_data_dispatch.py` — says less, is entirely upstream-sourced, and is executable. That is the trade this directive is asking for: **if a claim cannot be grounded in an admissible source, the useful version of it is the smaller one that can be.**
