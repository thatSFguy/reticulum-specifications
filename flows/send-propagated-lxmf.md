...
Same as `send-opportunistic-lxmf.md` step 1 except the desired_method differs. The router's `handle_outbound` (`LXMF/LXMRouter.py:1746-1795` → `def handle_outbound(self, lxmessage)`) will eventually route this to the propagation pipeline.

### 2. `LXMessage.pack()` — propagation-specific encryption

`LXMF/LXMessage.py:426-441` → `elif self.desired_method == LXMessage.PROPAGATED:`. Differences from the opportunistic and direct paths:
...
`LXMF/LXMRouter.py:2682-2899` → `def process_outbound(self, sender = None)` (the `PROPAGATED` branch is structurally similar to the `DIRECT` branch in `send-link-lxmf.md`). High-level state:
...
If the propagation node's announce app_data declared a `stamp_cost` (element [5][0] per §5.8.5), the sender computes a propagation stamp via `LXMessage.get_propagation_stamp(target_cost)` (`LXMessage.py:329-353` → `def get_propagation_stamp(self, target_cost, timeout=None)`). Algorithm same as §5.7 stamps but with `WORKBLOCK_EXPAND_ROUNDS_PN = 1000` rounds (cheaper than the regular `3000`-round stamp), and computed over the `transient_id` rather than `message_id`.
...
If `representation == PACKET`, the sender emits a Link DATA packet (`context = NONE`) carrying `propagation_packed`. The propagation node's `propagation_packet` callback (`LXMRouter.py:2234-2264` → `def propagation_packet(self, data, packet)`) decodes the msgpack outer, extracts the LXMF bodies, validates the propagation stamp if required, and stores each one in `propagation_entries[transient_id]`.
...
| 2 | `LXMF/LXMessage.py` | `pack` PROPAGATED branch, line 426-441 |
| 3 | `LXMF/LXMRouter.py` | `process_outbound`, line 2682-2899 |
| 4 | (see `send-link-lxmf.md` steps 3-4) | |
| 5 | `LXMF/LXMessage.py` | `get_propagation_stamp`, line 329-353 |
| 5 | `LXMF/LXStamper.py` | `WORKBLOCK_EXPAND_ROUNDS_PN = 1000` |
| 6 | `LXMF/LXMRouter.py` | `propagation_packet`, line 2234-2264 |
| 7 | `LXMF/LXMRouter.py` | `propagation_resource_concluded`, line 2336-2467 |
| 9 | `LXMF/LXMRouter.py` | `request_messages_from_propagation_node`, line 502-544 |
