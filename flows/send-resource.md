...
`RNS/Resource.py:249-485` → `def __init__(self, data, link, metadata=None`. In order:
...
`RNS/Resource.py:517-559` → `def advertise(self)`. Runs in a daemon thread because the build above can take a noticeable time on a large resource.
...
`RNS/Resource.py:585-598` → `if self.status == Resource.ADVERTISED:`. While `status == ADVERTISED` and no RESOURCE_REQ has arrived, the watchdog retransmits the advertisement up to `MAX_ADV_RETRIES = 4` times. After the 4th retry without a request, the resource is cancelled with status `FAILED` and a callback to the caller.
...
`RNS/Resource.py:994-1088` → `def request(self, request_data)`. The receiver has parsed the advertisement, accepted it, and now requests an initial window's worth of parts via a RESOURCE_REQ packet:
...
The initiator's `request()` handler at `RNS/Resource.py:1039-1088` → `if wants_more_hashmap:`:
...
The initiator's `validate_proof(proof_data)` (`RNS/Resource.py:796-839` → `def validate_proof(self, proof_data)`):
...
`RNS/Resource.py:573-683` → `def watchdog_job(self)`. The Resource owns a watchdog thread that runs through the lifecycle and adjusts timeouts based on observed link RTT. Key points for interop:
