...
- **2026-06-19 — §5.9.8 tap-back reactions now have an official upstream allocation at `FIELD_REACTION = 0x40`, which differs from the `fields[0x10]` app-extension the spec previously documented.**
  Until this revision §5.9.8 documented reactions only as a convergent app-extension at `fields[0x10]` (= `16`) with a *string-keyed* dict (`{"reaction_to", "emoji", "sender"}`), noting "there is no `FIELD_REACTION` constant in `LXMF/LXMF.py`." LXMF 1.0.0 (2026-05-28) added one: `FIELD_REACTION = 0x40` (`LXMF/LXMF.py:25`), carrying an *integer-keyed* dict `{REACTION_TO(0x00): raw-bytes message_id, REACTION_CONTENT(0x01): UTF-8 reaction}` with **no in-dict sender** (attribution is the carrying message's own source identity). The two shapes differ in field key (`0x10` vs `0x40`), inner-key type (string vs int), hash encoding (hex string vs raw bytes), and sender handling. An implementer who built against the old §5.9.8 emits `0x10` reactions that upstream LXMF 1.0.x (and clients tracking it) will not recognise as reactions, and ignores inbound `0x40` reactions from those clients. §5.9.8 now documents only the upstream `0x40` form (emit this); the non-upstream `0x10` form is retained for inbound parsing only, quarantined in a clearly-labeled non-normative §5.9.12 ("Deployed third-party field variants — not in upstream"). At time of writing MeshChatX has fully migrated to `0x40` (dropped `0x10`), Columba emits `0x40` with `0x10` as a fallback for un-upgraded peers, and `reticulum-mobile-app` is the remaining client still on `0x10`. Pins moved to `rns==1.3.5` / `lxmf==1.0.1` in the same revision; the related §5.9.9 reply-to keys (`0x30`/`0x31`) were also blessed upstream (matching the documented shape exactly) and §5.9.10/§5.9.11 added for the new `FIELD_COMMENT` (`0x41`) and `FIELD_CONTINUATION` (`0x42`).
