...
def get_private_key(self):
    ...
    #      ^^^^^^^^^^^^^   X25519 priv (set at line 708 from X25519PrivateKey.generate())
    #                       ^^^^^^^^^^^^^^^   Ed25519 priv (set at line 711)
```

`Identity.load_private_key(prv_bytes)` at line 738-751 slices it back the same way:
...
`Identity.from_bytes(prv_bytes)` at line 624-637 takes the same 64-byte concat and reconstitutes an `Identity`. The upstream docstring explicitly warns:
...
new_raw = bytes([raw[0] | 0x80, raw[1]]) + ifac + raw[2:]   # 0x80 = bit 7; IFAC sits between header and addresses
...
...
This is only partly redundant with interface framing. `TCPInterface` has long refused frames of `frame_len <= HEADER_MINSIZE` (`RNS/Interfaces/TCPInterface.py:338` → `if   frame_len <= RNS.Reticulum.HEADER_MINSIZE:        return False`), but `HEADER_MINSIZE` is 19, so that bound never caught a zero-data **HEADER_2** frame — at 35 bytes it clears the check comfortably. The unpack-time rule closes that case and applies on every interface, including the ones that frame without a length floor at all (KISS, RNode, serial). Implementations SHOULD reject a zero-length data field on receive; verified by `tools/verify_ingress_bounds.py`.

### 2.3 Originator HEADER_1 → HEADER_2 conversion

This is non-obvious and matters: when an **originator** (not a relay) sends a packet to a destination known to be more than 1 hop away, the originator MUST also do the HEADER_2 conversion. From `RNS/Transport.py::outbound` (lines 1345-1376 in RNS 1.5.2; verified by `tools/verify_packet_header.py`):
...
`random_hash` is **NOT** 10 random bytes — only the first 5 bytes are random; the trailing 5 bytes carry the emission timestamp as a big-endian unsigned 40-bit Unix-seconds integer (`RNS/Destination.py::announce` → `random_hash =`):
...
These are the MUST rules a receiver applies to every inbound announce before considering the announced destination "known". The canonical implementation is `RNS/Identity.py::validate_announce` (line 511-614 in RNS 1.5.2); the dispatch site that calls it is `RNS/Transport.py::_inbound` line 2171-2178 (`::inbound` through RNS 1.4.2 — see §13.1).

**Ingress bound (RNS ≥ 1.5.2).** Before any of the steps below runs, an inbound announce is size-checked against the protocol MTU: `Transport.preprocess_inbound` drops an `ANNOUNCE` whose raw frame exceeds `Reticulum.MTU` (500) and counts a protocol violation against the receiving interface (`RNS/Transport.py:1804` → `Excessive announce packet frame size`).

That is not redundant with the general per-interface frame bound a few lines earlier (`RNS/Transport.py:1791` → `Frame size exceeded MTU of`), which is measured against `interface.HW_MTU` — a bitrate-derived value reaching 524288 on a 1 Gbps link (`RNS/Interfaces/Interface.py:252` → `self.HW_MTU = 524288`). Through RNS 1.5.0, then, an announce of up to half a megabyte arriving on a fast interface was carried all the way into signature validation before anything discarded it; since 1.5.2 anything over 500 bytes is refused first, and step 2 below never runs on it.

The bound has always held on the **emit** side: `Packet.pack` raises when the packed frame exceeds the packet's MTU (`RNS/Packet.py:239` → `raise IOError("Packet size of "`), which for any non-LINK destination is `Reticulum.MTU` (`:157` → `self.MTU     = RNS.Reticulum.MTU`). A conformant sender cannot produce an oversized announce, so the receive-side rule exists for the senders that aren't conformant. Implementations SHOULD enforce both halves; verified by `tools/verify_ingress_bounds.py`.
...
Reject the announce iff `expected_hash != packet.destination_hash` (the value from the outer header). This catches both random hash collisions and active spoofing attempts that pair a valid signature with an unrelated dest_hash. (`RNS/Identity.py:565-567` → `hash_material = name_hash+announced_identity.hash`).

#### 4. Public-key collision rejection

If the receiver already has a different public_key cached for this `destination_hash` (from a prior announce), the new announce MUST be rejected with a critical-severity log even if the signature is otherwise valid. Per the upstream comment: "In reality, this should never occur, but in the odd case that someone manages a hash collision, we reject the announce" (`RNS/Identity.py::validate_announce` → `if destination_hash in`).
...
announce is still dropped. Note the return type is no longer strictly
boolean: a caller that tests `if validate_announce(...) == False` still
works, but `if not validate_announce(...)` does **not** catch the
blackholed case, because a non-empty string is truthy.
...
3. **`path_table`** entry update or insertion (see §4.6 — TBD when the relay rebroadcast spec lands), gated by:
   ...
   - Hop count comparison against any existing entry: equal-or-fewer hops always win; more hops win only if the cached path has expired or the new announce's emission timestamp (from `random_hash[5:10]`) is more recent than every cached blob's timestamp (`RNS/Transport.py:2253-2288` → `# If an announce`).
...
| `RNS/Transport.py::_inbound` → `if packet.packet_type == RNS.Packet.ANNOUNCE:` | inbound dispatch for `packet_type == ANNOUNCE`: quick sig check, ingress limiting, path table population, handler dispatch |
...
```python
# LXMRouter.lxmf_delivery, line 1848-1859
...
Official as of LXMF 1.0.0 (`FIELD_COMMENT = 0x41`, `LXMF/LXMF.py:26` → `FIELD_COMMENT = 0x41`). Marks the carrying message as a comment on another message. The comment text itself is carried as the normal LXM `content`, so a client that doesn't support comments simply renders it as an ordinary message (upstream rationale, `LXMF/LXMF.py:112-117` → `# Clients choose how to handle messages intended`).
...
Only the responder's X25519 is fresh-ephemeral; the responder signs with its **long-term** Ed25519 private key (asymmetric with the initiator). The responder's long-term Ed25519 public key is **not** sent on the wire — both sides already know it from the responder's prior announce, and it is included implicitly in the signature input. Signature input (`RNS/Link.py::prove` → `signed_data =` for the signer, `RNS/Link.py::validate_proof` → `signed_data =` for the validator):
...
This is asymmetric to the LINKREQUEST that established the link. LINKREQUEST is destination-hash-routed via `path_table` and therefore eligible for `HEADER_2` with `transport_id` set to the next-hop relay; the relay's path_table-forwarding branch strips `transport_id` (HEADER_2 → HEADER_1) at the last hop (`RNS/Transport.py::_inbound` → `if packet.transport_id != None and`):
...
A HEADER_2 link DATA packet would therefore arrive at the destination with `transport_id` intact, where the receiver's `Transport.packet_filter` (`RNS/Transport.py::packet_filter` → `if packet.transport_id != None`) drops it as "for another transport instance" because the embedded `transport_id` is the relay's identity, not the receiver's:
...
The `derived_key_length` at `RNS/Link.py:353-354` → `if self.mode == Link.MODE_AES128_CBC:` is what the HKDF in §6.4 produces, split as `signing_key(32) || encrypt_key(32)` for the AES-256 path or `signing_key(16) || encrypt_key(16)` for the AES-128 path.
...
The initiator reads `confirmed_mtu` back via `mtu_from_lp_packet` during LRPROOF validation (`RNS/Link.py:405-408` → `if self.initiator and len(packet.data)`), accepts it as `link.mtu`, and the link's `mdu` (max data unit per packet for §3.1 link-derived Token traffic) is recomputed via `update_mdu()`.
...
Receivers MUST handle both forms. `validate_request` at `RNS/Link.py:186-190` → `def validate_request(owner,` checks `len(data) == ECPUBSIZE` OR `len(data) == ECPUBSIZE+LINK_MTU_SIZE` and rejects anything else. The same length-dispatch is in `validate_proof` for the LRPROOF side at `RNS/Link.py:405-410` → `if self.initiator and len(packet.data)`. There is no flag bit signalling presence — wire length is the only signal.
...
There is also an explicit-cleanup path: after a STALE-induced teardown the watchdog adds a final grace period of `RTT × KEEPALIVE_TIMEOUT_FACTOR + STALE_GRACE` (= `RTT × 4 + 5s`) at line 754 to allow a delayed reply to bring the link back into ACTIVE before final teardown — but in upstream RNS 1.5.2 the `STALE → CLOSED` transition runs immediately on the next watchdog pass without consulting that grace period. The grace constant lives in case a future revision restores the soft-stale window.
...
> the reader was always bounded by `MAX_DATA_LEN`, not by the writer's
> figure. Implementations MUST size the reader's bound from
> `MAX_DATA_LEN`, not from whatever the peer's writer happens to emit.
...
The receiver builds `unique_tag = destination_hash || tag_bytes` and consults `Transport.discovery_pr_tags` (`Transport.py::preprocess_inbound` → `wait_start =`):

```python
unique_tag = destination_hash + tag_bytes

with Transport.discovery_pr_tags_lock:
    if (not unique_tag in Transport.discovery_pr_tags and
        not unique_tag in Transport.discovery_pr_tags_prev):
        Transport.discovery_pr_tags.add(unique_tag)
        tag_valid = True

if not tag_valid:
    return          # ignore duplicate path request
```

> **Changed in RNS 1.5.0.** Two things moved. First, the gate now runs
> in `Transport.preprocess_inbound` — on the ingress path, before the
> packet is queued for processing — rather than in
> `Transport.path_request_handler` after dispatch (§7.2.1); a duplicate
> is now discarded before it can occupy a slot in the inbound path-request
> queue. Second, `discovery_pr_tags` changed from a **list** to a **set**
> with a second generation, `discovery_pr_tags_prev`: a tag counts as
...
> (`Transport.py:850-853` → `if len(Transport.discovery_pr_tags)`) instead of ageing entries out individually.
...
2. **Path is known via the path_table AND `(transport_enabled OR is_from_local_client)`** (line 3459-3522): retrieve the cached announce packet from the path table, set its hops to the cached value, and queue it for retransmit. If the next hop happens to be the requestor itself (path-loop indicator), drop instead. This is the transport-mode path-resolver: a relay that already knows where the destination lives answers on its behalf, saving the requester from another hop of broadcast.
...
Upstream `Destination.rotate_ratchets()` (`RNS/Destination.py:228-235` → `def rotate_ratchets(self):`) runs on every announce but is a no-op unless `RATCHET_INTERVAL = 30*60s` has elapsed since the last rotation:
...
`RNS/Interfaces/TCPInterface.py` line 535 sets `self.OUT = False` in the `TCPServerInterface` constructor. This is overridden to `True` by `RNS/Reticulum.py` post-init at line 940-941 for any interface declared in the rnsd config:
...
`DETECT_REQ = 0x73` and `DETECT_RESP = 0x46` are at `RNode_Firmware/Framing.h:99-100`. The two-byte exchange tells a host "yes, this thing on the other end of the serial port is an RNode and it's awake". The host follows up immediately with `CMD_FW_VERSION`, `CMD_PLATFORM`, `CMD_MCU` queries — those queries each have a single `0x00` placeholder byte payload (per `RNodeInterface.detect()` line 487) and the RNode replies asynchronously with the same command code carrying the actual answer.
...
5. **Hash and proof material** (`Resource.py:445-448` → `self.random_hash =`). All three are computed over the **original uncompressed `plaintext`** — the caller's input, including any metadata prefix from step 1 (`Resource.py:262-268` → `packed_metadata = umsgpack.packb(metadata)`) — *not* the compressed body, and *not* the random-prefixed wire blob from step 3:
...
The flags byte `f` packs six booleans (`Resource.py:1318, 1367-1372` → `self.f = 0x00 | self.x`):
...
The advertisement is sent once on `Resource.advertise()`; if no part requests arrive within the watchdog timeout, it is retransmitted up to `MAX_ADV_RETRIES = 4` times before the resource is cancelled (`Resource.py:582-599` → `while self.watchdog_lock:`).
...
Sent by the receiver to ask for a window's worth of specific parts (`Resource.py:934-984` → `if self.very_slow_rate_rounds`). Body layout:
...
Receivers who already have the part for a requested map_hash don't issue requests for it; the request is constructed only from `parts[search_start:search_start+window]` where `parts[i] is None` (`Resource.py:950-966` → `requested_hashes = b""`).

### 10.6 RESOURCE part packets

For each map_hash in a RESOURCE_REQ, the sender locates the matching pre-packed part within `parts[receiver_min_consecutive_height : receiver_min_consecutive_height + COLLISION_GUARD_SIZE]` and emits it as a regular Link DATA packet with `context = RESOURCE (0x01)` (`Resource.py:1009-1020` → `requested_hashes = request_data[pad+RNS.Identity.HASHLENGTH//8:]`). The body is just the part's encrypted data — no metadata, no sequence number. The receiver matches the inbound part to its hashmap by recomputing its 4-byte map_hash and inserting it into `parts[i]` at the position where `hashmap[i]` matches (`Resource.py:875-891` → `part_hash = self.get_map_hash(part_data)`).
...
1. **Map_hashes are not guaranteed unique across the whole resource** — only within `COLLISION_GUARD_SIZE` of any sliding-window position. A receiver that searches the entire hashmap for a matching part-hash can mis-place a part if two distant parts collide. The reference receiver searches only `hashmap[consecutive_completed_height+1 : consecutive_completed_height+1+window]` (`Resource.py:877-879` → `search_start = self.consecutive_completed_height+1`).
   ...
   > part at that index was re-examined on every inbound part and the window
   > effectively covered one fewer outstanding slot than intended. 1.5.0
   > starts the scan at `consecutive_completed_height + 1` and drops the
   > now-redundant pointer bump inside the match branch. A receiver that
   > implemented the old bounds still interoperates — the window is a
   > receiver-private search hint, not a negotiated value — but will
   > mis-place a part slightly more often under collision.
...
> `Resource.py:616-634` → `if self.req_resp_rtt_rate !=`:
>
> ```python
...
If the part_index doesn't land on a `HASHMAP_MAX_LEN` boundary, the sender treats it as a sequencing error and cancels the resource (`Resource.py:1051-1054` → `if part_index % ResourceAdvertisement.HASHMAP_MAX_LEN`).
...
> `Resource.py:490` → `if self.waiting_for_hmu:`), and an HMU whose hashmap segment decodes to
> zero map-hashes cancels the transfer (`Resource.py:506-510` → `if hashes < 1:`). The
> sender likewise cancels instead of emitting an empty hashmap
> continuation (`Resource.py:1064-1067` → `if not hashmap:`). Implementations should not
...
>
> In the RNS reference (`Resource.py::request_next` → `self.rtt_rxd_bytes_at_part_req`, `request()` — verified
> against RNS 1.5.2, the current release), the part-fulfilment loop runs
...
> (`request_next`, `Resource.py:934-984` → `if self.very_slow_rate_rounds`) routinely produces this
...
When the receiver has assembled the full resource (`received_count == total_parts`), it runs `assemble()` (`Resource.py:685-764` → `def assemble(self):`):
...
> prefix" — sender-side `Resource.py:402, 408` → `self.uncompressed_size =` writes those bytes via
...
sent as `RNS.Packet(link, proof_data, packet_type=PROOF, context=RESOURCE_PRF)` (`Resource.py:756-766` → `if hasattr(self.data,`). The `full_proof` is exactly what the initiator pre-computed as `expected_proof` in §10.2 step 5 — it can validate the proof bytewise without re-running the SHA-256.

The initiator's `validate_proof` (`Resource.py:796-841` → `def validate_proof(self,`) checks `proof_data[32:] == self.expected_proof` and transitions status to `COMPLETE`. If the resource is multi-segment (`s == True`), the next segment's advertisement is sent immediately upon proof of the current segment.
...
After each successful round (every requested part arrived), `window += 1` up to `window_max`; `window_min += 1` once `window - window_min > WINDOW_FLEXIBILITY - 1` (`Resource.py::receive_part` → `RNS.log("Error`). The window cap is promoted to `WINDOW_MAX_FAST` after `FAST_RATE_THRESHOLD` consecutive rounds at observed throughput > `RATE_FAST = 50 kbps / 8`, and demoted to `WINDOW_MAX_VERY_SLOW` after `VERY_SLOW_RATE_THRESHOLD = 2` rounds below `RATE_VERY_SLOW = 2 kbps / 8` (`Resource.py::receive_part` → `self.rtt_rxd_bytes_at_part_req =`). These are receiver-private — they're not negotiated, so two implementations with different rate-detection cutoffs interop fine but may emerge with different effective throughput on the same channel.

### 10.11 Multi-segment resources

For payloads larger than `MAX_EFFICIENT_SIZE = 1 MiB - 1`, the resource is split into multiple segments at `MAX_EFFICIENT_SIZE` boundaries (`Resource.py:307-322` → `((self.total_size-1)//Resource.MAX_EFFICIENT_SIZE)+1`). Each segment is its own Resource with its own RESOURCE_ADV; the `i` (segment_index) and `l` (total_segments) fields disambiguate. The `o` (original_hash) field carries the first segment's `h` so the receiver can correlate segments belonging to the same logical transfer.

The sender doesn't pre-prepare every segment up front — it builds segment N+1 in `__prepare_next_segment` while segment N is still being delivered, and sends segment N+1's advertisement only after it has received the proof for segment N (`Resource.py::prove` → `if not self.ensure_link():`). This caps memory usage; a 100 MiB transfer doesn't materialize 100 segments simultaneously.
...
```python
# RNS/Reticulum.py:157, 1239-1245
...
> the link RTT and the outstanding window (`Resource.py:127-135` → `PART_TIMEOUT_FACTOR =`,
> `:565-674` → `if self.req_data_rtt_rate !=`). One `MAX_EFFICIENT_SIZE` segment on a slow link can legitimately
...
The `.meta` sidecar written for a metadata-carrying first segment (`Resource.py:713-715` → `metadata_file = open(self.meta_storagepath, "wb")`) is unlinked when the final segment concludes (`:746` → `try: os.unlink(self.meta_storagepath)`), but it is **not** swept: `__clean_caches` only considers filenames of exactly `(HASHLENGTH//8)*2 = 64` characters (`Reticulum.py:1241`), and `{o.hex()}.meta` is 69. An abandoned multi-segment transfer leaves its metadata file behind permanently.
...
> `get_hashable_part()` (`Packet.py:361-366` → `def get_hashable_part(self):`) is:
>
> ```
...
- Enables the §12.2 DATA forwarding branches in `Transport._inbound`.
...
Increment hops (already done by `Transport.preprocess_inbound` line 1800), replace the transport_id with the next-hop transport_id from the path table, keep the rest of the packet:
...
When the forwarded packet is a `LINKREQUEST`, the relay also writes a `link_table` entry keyed by the link_id (computed via §6.3's `link_id_from_lr_packet`). Entry contents (`Transport.py:2090-2098` → `link_entry = [ now,`):
...
For any other forwarded DATA (the much-more-common opportunistic LXMF case), the relay writes a `reverse_table` entry keyed by `packet.getTruncatedHash()` (`Transport.py:2015-2018` → `# If the packet is`):
...
When an inbound ANNOUNCE validates (per §4.5) AND the destination is non-local AND `transport_enabled OR is_from_local_client`, the relay queues a rebroadcast. From `Transport.py:2253-2311` → `# If an announce`:
...
§4.5 step 6.3 already documents this from the receiver's perspective; for the rebroadcast logic, the relay only queues an announce if the new `random_blob` (the 10-byte `random_hash` field, treated as an opaque blob for routing purposes) is **not** already in the cached `random_blobs` list for this destination. The list is capped at `Transport.MAX_RANDOM_BLOBS` (default 64) entries, sliding-window. This prevents an announce from looping through a multi-relay topology because each relay only forwards each unique blob once.

#### 12.3.3 Path-response announces don't rebroadcast

`packet.context == PATH_RESPONSE` short-circuits the rebroadcast branch (line 2351). Path-response announces travel back along the reverse path from the responder to the requester (see §7.2 and `flows/path-discovery.md`), and the relay's job is to forward them on a single specific interface (`attached_interface`), not re-broadcast to the whole mesh. Mishandling this would multiply path-response traffic by the relay fanout.

### 12.4 Path table management

`Transport.path_table[destination_hash]` entry shape (`Transport.py:4122-4128` → `IDX_PT_TIMESTAMP =`):
...
`Transport.jobs` runs a `stale_paths` accumulator that walks `path_table` and pops entries whose `expires` timestamp has passed (`Transport.py:957-978` → `# Cull the path table`). Eviction is silent — no notification to the application; the next outbound message to the destination just re-discovers it via `request_path` per §7.1.
...
> **Changed in RNS 1.5.0 — two forwarding-behaviour changes.**
>
> 1. **Pre-validation link packets are now dropped.** Before forwarding,
>    the relay checks `link_entry[IDX_LT_VALIDATED]`; if the link's LRPROOF
>    has not yet been validated by §12.5.1, the packet is discarded and
>    counted as a protocol violation against the receiving interface
>    (`RNS/Transport.py:2126-2128`). Through RNS 1.4.2 the relay forwarded
>    link DATA in both directions from the moment the `link_table` entry
>    was written by §12.2.4, i.e. before the responder had proved the link.
>    An initiator that starts sending link DATA before it receives the
>    LRPROOF will now have those packets dropped at the relay rather than
>    forwarded; this is not a wire-format change, but it does make the
>    §6.2 handshake ordering load-bearing over multi-hop paths.
> 2. **The branch now returns.** After link_table forwarding, `_inbound`
>    returns instead of falling through to the remaining handling
>    (`RNS/Transport.py:2166`). The `return` was present but commented out
>    through 1.4.2 with the note *"Can we return safely here?"*. A
...
- Path-table entries with `IDX_PT_HOPS == 0` mean "destination is a local client" — the §2.3 originator-side HEADER_1 conversion applies for hops==1 too, so the shared instance gets a transport_id-tagged packet (`Transport.py:1416-1427` → `elif path_entry[IDX_PT_HOPS]`).
...
| File | What |
|---|---|
| `RNS/Transport.py::_inbound` → `if packet.transport_id != None and` | DATA forwarding (HEADER_1↔HEADER_2 conversion for relay) |
...
#### 13.1.1 The prioritized ingress queue (RNS ≥ 1.5.0)

Through RNS 1.4.2, `Transport.inbound(raw, interface)` was one function: it
authenticated IFAC, unpacked the packet, ran `packet_filter`, and then handled
or forwarded it — all on the calling interface's RX thread. RNS 1.5.0 splits
that into a three-stage pipeline with a queue in the middle:
...
|---|---|---|---|
| `TC_DATA` | `0x00` | `INBOUND_DA_QUEUE_LENGTH = 4096` | Everything not classified below |
| `TC_ANNOUNCE` | `0x01` | `INBOUND_AN_QUEUE_LENGTH = 256` | `packet_type == ANNOUNCE` |
| `TC_PATH_REQUEST` | `0x02` | `INBOUND_PR_QUEUE_LENGTH = 256` | Packets addressed to the path-request destination (§7.1) |
| `TC_INGRESS_LIMITED` | `0x03` | `INBOUND_IL_QUEUE_LENGTH = 128` | Traffic from an interface currently over its ingress limit |

When a queue is full, the packet is dropped at enqueue time rather than
blocking the RX thread. `Transport.USE_INBOUND_QUEUE` (default `True`) selects
this path; the symmetric `USE_OUTBOUND_QUEUE` exists but is `False` and
`Transport.outbound_job` raises `NotImplementedError`, so egress is still
synchronous through `Transport._outbound`.

**None of this is on the wire.** A clean-room implementation is free to keep a
single-stage receive path. What it inherits from the split is the same
invariant the announce-handler row above states: slow handling of one packet
class must not stall another. The queue lengths are the upstream answer to
that, and they are also the upstream answer to a flood — a burst of announces
can fill `INBOUND_AN_QUEUE_LENGTH` and be dropped without ever touching the
4096-deep data queue.
...
- **`AnnounceHandler.received_announce`** — fires once per accepted announce, **on a fresh daemon thread**. This is the only callback that's NOT on the receive thread (`Transport.py:2506-2535` → `if execute_callback:`).
...
| `Transport.discovery_pr_tags` rotation (`PATH_REQUEST_GATE_TIMEOUT = 45s`) | §7.2.2 | Path-request dedup table never evicts old entries |
...
| `path_table[dest][IDX_PT_RANDBLOBS]` (per-destination random_blob history) | `Transport.MAX_RANDOM_BLOBS = 64` (RNS 1.5.2) | §4.5 step 6.3, §12.3.2 | Sliding window. Caps memory growth from one destination's announce stream. |
...
| `Transport.path_requests` | (unbounded — one entry per recently-issued path? request) | §7.1 | Aged out at `Transport.PATH_REQUEST_GATE_TIMEOUT = 45s` (`120s` through RNS 1.4.2). |
